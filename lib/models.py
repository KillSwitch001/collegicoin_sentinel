...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, collegicoind):
        golist = collegicoind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_collegicoind(collegicoind, item)
    ...
    @classmethod
    def import_gobject_from_collegicoind(self, collegicoind, rec):
        import decimal
        ...
        # shim/collegicoind conversion
        object_hex = collegicoinlib.SHIM_deserialise_from_collegicoind(object_hex)
        objects = collegicoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from collegicoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from collegicoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from collegicoind - Collegicoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(collegicoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from collegicoind! %s" % e)
            govobj.vote_delete(collegicoind)
            return (govobj, None)
    ...
    def vote_delete(self, collegicoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(collegicoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, collegicoind, signal, outcome):
        import collegicoinlib
        ...
        output = collegicoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = collegicoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(collegicoind, signal)

    def sync_network_vote(self, collegicoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = collegicoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 collegicoin addr, non-multisig
            if not collegicoinlib.is_valid_collegicoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Collegicoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (collegicoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/CollegicoinDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import collegicoinlib
        obj_data = collegicoinlib.SHIM_serialise_for_collegicoind(self.serialise())
    ...
    def prepare(self, collegicoind):
        try:
            object_hash = collegicoind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not collegicoinlib.is_valid_collegicoin_address(addr, config.network):
    ...
    def hash(self):
        import collegicoinlib
        return collegicoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, collegicoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - collegicoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, collegicoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - collegicoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, collegicoind):
        now = int(time.time())
        return (self.created_at < (now - collegicoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, collegicoind):
        if self.is_expired(collegicoind):
            return False

        return True

    def is_deletable(self, collegicoind):
        if self.is_expired(collegicoind):
            return True
