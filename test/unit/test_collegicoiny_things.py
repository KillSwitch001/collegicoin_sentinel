...
def test_valid_collegicoin_address():
    from collegicoinlib import is_valid_collegicoin_address

    main = valid_collegicoin_address()
    test = valid_collegicoin_address('testnet')

    assert is_valid_collegicoin_address(main) is True
    assert is_valid_collegicoin_address(main, 'mainnet') is True
    assert is_valid_collegicoin_address(main, 'testnet') is False

    assert is_valid_collegicoin_address(test) is False
    assert is_valid_collegicoin_address(test, 'mainnet') is False
    assert is_valid_collegicoin_address(test, 'testnet') is True


def test_invalid_collegicoin_address():
    from collegicoinlib import is_valid_collegicoin_address

    main = invalid_collegicoin_address()
    test = invalid_collegicoin_address('testnet')

    assert is_valid_collegicoin_address(main) is False
    assert is_valid_collegicoin_address(main, 'mainnet') is False
    assert is_valid_collegicoin_address(main, 'testnet') is False

    assert is_valid_collegicoin_address(test) is False
    assert is_valid_collegicoin_address(test, 'mainnet') is False
    assert is_valid_collegicoin_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from collegicoinlib import elect_mn
...
def test_parse_masternode_status_vin():
    from collegicoinlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % collegicoinlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(collegicoinlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(collegicoinlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(collegicoinlib.blocks_to_seconds(16616)) == 2612035
